...
class Pipes implements GameObject {
    ...
    private Asset assetUp = new Asset("/images/Pipe/pipe-bottom.png", WIDTH, HEIGHT);
    private Asset assetDown = new Asset("/images/Pipe/pipe-top.png", WIDTH, HEIGHT);
    ...
    public void update(long now) {
        if (Osama.gameStarted) {
            for (int i = 0; i < spritesUp.size(); i++) {
                ...
                if (Osama.activePipes[0].getPosX() + WIDTH < screenWidth / 2 - 56) {
                    Osama.activePipes = new Sprite[] { spritesUp.get(i), spritesDown.get(i) };
                }
            }
        }
    }
